SELECT
	...
	,d.prd_no
	,e.SNM AS PRDT_SNM
	,CAST(e.SPC AS varchar) AS PRDT_SPC
	,orderQty
	,created
	,modified
FROM productionHistory.dbo.tbmkno d
	INNER JOIN DB_U105.dbo.PRDT e ON d.prd_no=e.PRD_NO
UNION
-- 由天心外掛擷取白瓶生產次數及相關內容
SELECT
	NULL AS id
	...
	,b.prd_no
	,c.SNM AS PRDT_SNM
    ,CAST(c.SPC AS varchar) AS PRDT_SPC
	,b.orderQty
	,NULL AS created
	,NULL AS modified
FROM (
